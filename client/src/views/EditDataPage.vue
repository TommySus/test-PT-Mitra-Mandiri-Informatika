<template>
  <div class="EditPage">
    ...
    <div class="EditBox">
      <div class="titleEditEmployee">
        <h2>Edit Data Karyawan</h2>
      </div>
      <div>
        <form class="inputForm" @submit.prevent="confirmationEditEmployee">
          <div class="inputName">
          ...
          <div class="buttonForm">
            ...
            <button class="EditNewEmployeeButton" type="submit">Simpan</button>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    fetchEmployee () {
      ...
      this.$store.dispatch('fetchEmployeeById', payload)
        .then(({ data }) => {
          this.employeId = data.id
          this.name = data.name
          this.birth_date = this.getDate(data.birth_date)
    ...
    editEmployee () {
      ...
      this.$store.dispatch('editEmployee', payload)
        .then(({ data }) => {
          Swal.fire(
            'Success!',
...
<style>
  ...
  .EditBox .titleEditEmployee{
    width: 100%;
    height: 100%;
    border-bottom: 3px solid black;
  }
  .EditBox .titleEditEmployee h2{
    float: left;
    margin-left: 10px;
  }
  .inputForm {
  ...
  .buttonForm .cancel, .buttonForm .EditNewEmployeeButton{
    width: 150px;
    margin-left: 30px;
    height: 40px;
    font-size: 20px;
  ...
  .buttonForm .cancel:hover, .buttonForm .EditNewEmployeeButton:hover {
    opacity: 1;
  }
